...
function fallbackChatChargeId(input: { sessionId: string; route: string; faculty: string; at: number }): string {
  const digest = createHash("sha256")
    .update(`${input.sessionId}:${input.route}:${input.faculty}:${input.at}`)
    .digest("hex")
    .slice(0, 14);
...
function streamStudentLine(args: {
  ...
  teacherSaid?: string;
  /** Server-generated avatar speech from a guided room turn. Human-written
   *  dialogue is rejected at the route boundary. See docs/player-dialogue-policy.md. */
  playerText?: string;
  ...
  const teacherSaidContext = args.teacherSaid && args.teacherSaid.trim()
    ...
    : "";
  // Quote only server-generated avatar speech from the guided room turn.
...
function manualClassroomTurnPlan(args: {
  ...
  faculty: string;
  intent?: PlayerChatIntent;
  /** Avatar line generated by the server for /room-turn. */
  playerLine?: string;
}): {
  ...
  if (args.intent === "hint") {
    ...
    directive = args.intent === "report" || classReportBlocksBoard
      ...
      : args.intent === "advance" && schedulerControlsBoard
      ? `The player chose to advance. React in ONE short sentence. The Ruby High scheduler will put the next card on the board after your reply. ${schedulerBoundaryInstruction(bank)}`
      : args.intent === "advance"
      ? `The player chose to advance. React in ONE short sentence, then put a fresh challenge on the board. ${requiredNextBoardInstruction(bank, "Call pick_from_bank exactly once to put the next scheduled question on the board.")}`
      : schedulerControlsBoard
...
export async function handleChatRoutes(ctx: ChatRouteContext): Promise<boolean> {
  ...
  // Child-privacy boundary: human players choose intents and card IDs. The
  // server supplies the dialogue. Keep this retired endpoint closed so an old
  // client cannot publish or send player prose to a model. See the dialogue policy.
  if (ctx.method === "POST" && ctx.pathname === CHAT_PREFIX) {
    ctx.error(ctx.res, "Choose a chat action or response cards.", 410);
    return true;
  }

  // Child privacy: an intent ID starts this turn; prose comes from the server.
  ...
  if (ctx.method === "POST" && ctx.pathname === `${CHAT_PREFIX}/event`) {
    ...
    // Older clients sent generated player lines here. Accept actions only;
    // server-owned /room-turn creates all student speech under the privacy rule.
    if (body?.context?.playerLine != null) {
      ctx.error(ctx.res, "Choose a chat action or response cards.", 400);
      return true;
    }
    ...
    if (trigger === "channel-enter") {
      ...
      const round = state.activeRound;
      // Use saved curriculum and outcomes for model context. Client-supplied
      // prose fields are compatibility data only; the question ID guards staleness.
      const resolved = buildResolvedAnswerBriefing({ state, playerName });
  ...
  if (ctx.method === "POST" && ctx.pathname === `${CHAT_PREFIX}/student-chime`) {
    ...
    const body = (await ctx.readJsonBody().catch(() => ({}))) as
      ...
      | null;
    if (body?.playerText != null || body?.recordPlayerText) {
      ctx.error(ctx.res, "Choose a chat action or response cards.", 400);
      return true;
    }
    ...
    // Only game-defined situations reach the model. Older clients may send
    // note text; the saved class state supplies that context instead.
    const situation = body?.situation === "answer-correct" || body?.situation === "answer-wrong"
      ? body.situation : "ambient classroom moment";

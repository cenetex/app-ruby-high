...
describe("createStateStore", () => {
  ...
  it("defaults to JSON-file backend when nothing is set", () => {
  ...
  it("returns DynamoStateStore when explicitly configured via opts", () => {
    const store = createStateStore({
      backend: "dynamodb",
  ...
  it("returns DynamoStateStore when env opts in", () => {
  ...
  it("accepts the dynamo / ddb aliases on the backend env var", () => {
  ...
  it("throws a clear error when dynamodb is requested without a table name", () => {
    delete process.env.RUBY_HIGH_DYNAMO_TABLE;
    expect(() => createStateStore({ backend: "dynamodb" })).toThrow(/RUBY_HIGH_DYNAMO_TABLE/);
  });

  it("falls back to JSON for an unknown backend value, with a warning", () => {
  ...
  it("opts.jsonPath is wired through to the JSON backend", () => {
    const store = createStateStore({ backend: "json", jsonPath: "/tmp/explicit.json" });
    expect(store).toBeInstanceOf(StateStore);
    expect(store.describe()).toBe("/tmp/explicit.json");
  });

  it("ignores bad TTL env values and warns", () => {
    process.env.RUBY_HIGH_STORE_BACKEND = "dynamodb";
    process.env.RUBY_HIGH_DYNAMO_TABLE = "t";

...
describe("Roko", () => {
  it("ships the requested voice and a defensive information-hazard boundary", () => {
    ...
    expect(roko?.systemPrompt).toContain("dragon-and-goblin fables");
    expect(roko?.systemPrompt).toContain("Pascal's Mugging");
  ...
  it("ships a full course with goblin fables and incident case studies", async () => {
    ...
    expect(roko?.questions).toEqual(expect.arrayContaining([
      expect.objectContaining({ prompt: expect.stringContaining("dragon fable") }),

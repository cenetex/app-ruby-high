...
export function buildSessionState(args: {
  ...
  const telemetry: SessionTelemetry = {
    ...
    guest_pack: (() => {
      ...
      const summary = (p: ReturnType<typeof guestPackForSession>): PackSummaryTelemetry | null => {
        ...
        return {
          ...
          teacher_name: firstFaculty?.shortName || firstFaculty?.displayName || "Guest teacher",
          subject: firstFaculty?.subjects?.[0] || "Guest class",
    ...
    school_events: (Array.isArray(state.schoolEvents) ? state.schoolEvents : []).slice(-30),

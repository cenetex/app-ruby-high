...
## Follow-up — re-check after commit `a114320` (2026-05-31 14:32)

The working-tree changes were committed as `a114320` ("X social + Telegram integration, admin dashboard, and **security hardening**"), which incorporated this report and addressed most findings. Verified against the committed code (typecheck clean; all 19 x-social tests pass):

| Finding | Status after `a114320` | Evidence |
|---|---|---|
| **C1** admin SyntaxError | ✅ **Fixed** | Duplicate declaration block removed; one set of `studentsPanel`/`refreshStudents`/… remains. Admin panel un-bricked. |
| **C2** NFT provable-fairness | ❌ **Not addressed** | `pickPlayerStudentCard` (`ruby-high-service.ts:6650`) still selects from live `sessions` by `idx % candidates.length`; `hallPassCardPackEntries:6697` still injects it; `hallPassCatalogHash` still commits only the static catalog. *(Was flagged as a product decision — reasonable to defer, but it remains the open integrity gap.)* |
| **C3** bot-token leak | ✅ **Fixed** | `getConfig()` now returns `{ chatId, enabled, hasToken }` (token destructured out); GET fallback returns `hasToken: false`. |
| **H1** SSRF | 🟡 **Mostly fixed; residual** | `resolveImageToBuffer` now enforces https-only, blocks `localhost`/`.local`/`.internal`, checks literal IPs via a thorough `isBlockedAddress` (RFC1918/loopback/link-local/CGNAT/IPv6 ULA), and times out. **Gap:** no DNS resolution for hostnames — a public domain whose A-record points at a private IP (DNS rebinding) still passes. The repo's existing DNS-resolving `assertSafeMaterialsUrl` was not reused. *(Downgraded High→Medium.)* |
| **H2** predictable OAuth state | ✅ **Fixed** | `state = rh-x-${base64UrlEncode(randomBytes(16))}-${teacherId}` (`x-social-service.ts:327`). |
| **H3** transient-failure disconnect | ✅ **Fixed** | `ensureFreshToken` now returns `null` on failure paths instead of destructively `disconnect()`-ing. |
| **H4** `/x/status` unauthenticated | ✅ **Fixed** | `requireAdminAuth` gate added (`x-social.ts:101`). |
| **M1** missing fetch timeouts | ✅ **Addressed** | `AbortSignal.timeout` / `AbortController` added on Telegram, image, and X user fetches. |
| **M2** find-chat token in URL | ✅ **Fixed** | Validates `^\d+:[A-Za-z0-9_-]+$`, `encodeURIComponent`, timeout, 502 on failure. (Token still travels in the query string — minor log-exposure residual.) |
| **M5** non-atomic token write | ✅ **Fixed** | `writeFile(tmp)` → `rename(tmp, filePath)`. |

**Not re-verified this pass** (commit was scoped to security hardening; likely still open): M3 (rate-limit / daily-photo TOCTOU races), M4 (`import-token` `as any` reach-ins), M6 (NFT image-upload override path), M7 (two divergent `requireAdminAuth` impls), and the Low items (dead code, weakened regression test, `yearbook.ts` SVG escaping, Telegram HTML escaping).

**Remaining priorities:** (1) **C2** — decide player-card vs. provable-fairness before any pack mint that includes player cards; (2) **H1 residual** — add DNS resolution to the SSRF guard (or reuse `assertSafeMaterialsUrl`) to close DNS rebinding.

---
*Generated by automated multi-agent review (4 parallel reviewers across X-social, NFT/core-service, admin/telegram, and viewer/scripts subsystems) with manual verification of all Critical/High findings against the working tree. Follow-up section verified against commit `a114320`.*
